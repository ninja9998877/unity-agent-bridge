...
这是刻意设计的 —— 你不需要为了调试而改动任何生产代码：

| 维度 | 做法 |
|---|---|
| **业务代码** | **一行不改。** 读私有成员、调私有方法一律走反射 |
| **场景 / Prefab / 配置表** | 不动，也**不用往场景里挂 GameObject**（靠 `[InitializeOnLoad]` 常驻） |
| **工程设置** | 不动。`ProjectSettings/` 一个字节都不写 |
| **正式包** | 放 `Assets/Editor/` 下 → 编辑器专用编译，**打包自动排除** |
| **版本控制** | 指令与结果文件在 `Temp/`，不入库 |

唯一的「侵入」就是**新增两个 `.cs` 文件**。不想要了直接删掉，工程立刻回到原样。
...
| | OS 层自动化（截图 + 坐标点击） | Unity 官方测试框架 | **本工具** |
|---|---|---|---|
| 需要 OS 级操作 | ✅ 必须：鼠标注入、坐标标定 | ❌ | ❌ **不需要** |
| 需要 Unity 提供 API / 装包 | — | ✅ 要装包、配 asmdef | ❌ **不需要** |
| 版本门槛 | — | 绑定包版本 | **无版本上限**（实测 2020.3） |
| 能验证 UI 手感 / 渲染 | ✅ | ❌ | ❌（那是 OS 层的地盘） |

- **不需要 OS 级操作。** 不做坐标点击、不注入鼠标事件 —— 所以不受分辨率、窗口位置、DPI 标定的影响，
  也不会出现"点了但没点对地方"这类问题。
- **不需要 Unity 提供 API。** 不用装包、不用配 asmdef、不依赖测试框架。
  只用到 `EditorApplication.update` + 反射 + `System.IO` —— 这些是十几年没动过的稳定接口。
- **没有版本上限。** 没有任何一处依赖新版 Unity 才有的能力。

> 这不是要取代 OS 层自动化 —— 真机验证、UI 手感、渲染表现仍然是它的地盘。
> 本工具解决的是**编辑器内的场景复现**，两条路线互补。
